      ...
      >
        ...
        <b-row>
          ...
          <b-col cols="6">
            ...
            >
              <SearchField search-place-holder="Search declare of leaves" @searchValue="onSearch" />
            </form>
          </b-col>
          <b-col cols="3">
            <nuxt-link :to="{path: `/service_providers/`+ $route.params.provider_id +`/leaves/form/`, query: $route.query }" class="ml-2 mb-4 float-right">
        ...
        <b-row v-if="$store.getters.fetchedLeaves.status === 'REJECTED'">
          <b-col xs="12">
        ...
        <b-row>
          <b-col xs="12">
            <b-table
              :busy="$store.getters.fetchedLeaves.status === 'PENDING'"
              :items="items"
              :fields="fields"
            ...
            >
              ...
              <template v-slot:cell(action)="data">
                <nuxt-link :to="{path: `/service_providers/`+ $route.params.provider_id +`/leaves/form/${data.item.id}`, query: $route.query}">
...
export default {
  ...
  methods: {
    ...
    async fetch (params = {}) {
      ...
      await this.$store.dispatch('fetchLeaves', params)
      // fetched results came from vuex store getters
      const fetched = this.$store.getters.fetchedLeaves
      ...
      if (fetched.status === 'FULFILLED') {
        ...
        // Record Fields
        this.fields = [
          { key: 'note', label: 'Declare Leave', sortable: true },
    ...
    async handleDelete (id) {
      ...
      const deleted = this.$store.getters.deletedLeave
